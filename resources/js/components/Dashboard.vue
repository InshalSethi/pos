      ...
      <div v-else>
          ...
          <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow-sm hover:shadow-md transition-all rounded-2xl border border-zinc-200 dark:border-zinc-800 hover:border-black dark:hover:border-white">
            <div class="p-6">
              <div class="flex items-center justify-between">
                <div class="w-full">
                  <div class="flex items-center justify-between">
                    <div class="w-11 h-11 bg-slate-100 text-slate-600 dark:bg-zinc-800 dark:text-zinc-300 rounded-xl flex items-center justify-center shadow-xs">
                    ...
                    <span class="text-[11px] font-medium bg-slate-100 text-slate-700 dark:bg-zinc-800 dark:text-zinc-300 px-2.5 py-1 rounded-full border border-slate-200 dark:border-zinc-700">+2.5%</span>
                  </div>
                  <div class="mt-4">
                    <p class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-zinc-500">Total Sales</p>
                    <p class="text-2xl font-medium text-slate-800 dark:text-zinc-100 mt-0.5 tracking-tight">{{ formatAmount(dashboardData.sales?.total_amount || 0) }}</p>
                    <p class="text-[11px] text-slate-500 dark:text-zinc-400 mt-1">Vs previous period</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="flex items-center justify-between">
                <div class="w-full">
                  <div class="flex items-center justify-between">
                    ...
                    <span class="text-[11px] font-medium bg-slate-100 text-slate-700 dark:bg-zinc-800 dark:text-zinc-300 px-2.5 py-1 rounded-full border border-slate-200 dark:border-zinc-700">-2.1%</span>
                  </div>
                  <div class="mt-4">
                    <p class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-zinc-500">Total Expenses</p>
                    <p class="text-2xl font-medium text-slate-800 dark:text-zinc-100 mt-0.5 tracking-tight">{{ formatAmount(dashboardData.expenses?.total_amount || 0) }}</p>
                    <p class="text-[11px] text-slate-500 dark:text-zinc-400 mt-1">Vs previous period</p>
                  </div>
                </div>
              </div>
              ...
              <div class="flex items-center justify-between">
                <div class="w-full">
                  <div class="flex items-center justify-between">
                    ...
                    <span class="text-[11px] font-medium bg-slate-900 text-white dark:bg-zinc-100 dark:text-zinc-900 px-2.5 py-1 rounded-full">
                      {{ dashboardData.payments?.total_payments || 0 }} TXNS
                    </span>
                  </div>
                  <div class="mt-4">
                    <p class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-zinc-500">Total Payments</p>
                    <p class="text-2xl font-medium text-slate-800 dark:text-zinc-100 mt-0.5 tracking-tight">{{ formatAmount(dashboardData.payments?.total_amount || 0) }}</p>
              ...
              <div class="flex items-center justify-between">
                <div class="w-full">
                  <div class="flex items-center justify-between">
                    ...
                    <span class="text-[11px] font-medium bg-slate-200 text-slate-800 dark:bg-zinc-800 dark:text-zinc-200 px-2.5 py-1 rounded-full border border-slate-300 dark:border-zinc-700">
                      {{ dashboardData.payments?.pending_payments || 0 }} PENDING
                    </span>
                  </div>
                  <div class="mt-4">
                    <p class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-zinc-500">Pending Payments</p>
                    <p class="text-2xl font-medium text-slate-800 dark:text-zinc-100 mt-0.5 tracking-tight">{{ formatAmount(dashboardData.payments?.pending_amount || 0) }}</p>
                    <div class="mt-2">
                      <router-link
                        to="/payments?status=pending"
                        class="inline-flex items-center text-xs text-slate-800 dark:text-zinc-200 font-medium hover:underline group"
                      >
                        View Pending
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow-sm hover:shadow-md transition-all rounded-2xl border border-zinc-200 dark:border-zinc-800 hover:border-black dark:hover:border-white p-6">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-medium text-slate-800 dark:text-zinc-100 mt-2 tracking-tight">{{ formatAmount(dashboardData.inventory_valuation?.total_cost_value || 0) }}</p>
              </div>
          ...
          <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow-sm hover:shadow-md transition-all rounded-2xl border border-zinc-200 dark:border-zinc-800 hover:border-black dark:hover:border-white p-6">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-2xl font-medium text-slate-800 dark:text-zinc-100 mt-2 tracking-tight">{{ formatAmount(dashboardData.inventory_valuation?.total_retail_value || 0) }}</p>
              </div>
              <div class="w-11 h-11 bg-slate-100 text-slate-600 dark:bg-zinc-800 dark:text-zinc-300 rounded-xl flex items-center justify-center shadow-xs shrink-0 ml-3">
          ...
          <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow-sm hover:shadow-md transition-all rounded-2xl border border-zinc-200 dark:border-zinc-800 hover:border-black dark:hover:border-white p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-zinc-500">Potential Profit</p>
                <p class="text-2xl font-medium text-slate-800 dark:text-zinc-100 mt-2 tracking-tight">{{ formatAmount(dashboardData.inventory_valuation?.potential_profit || 0) }}</p>
              </div>
              <div class="w-11 h-11 bg-slate-100 text-slate-600 dark:bg-zinc-800 dark:text-zinc-300 rounded-xl flex items-center justify-center shadow-xs shrink-0 ml-3">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div class="bg-white dark:bg-zinc-900 shadow-sm rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800">
            <div class="px-6 py-4 bg-zinc-100 dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
              <h3 class="font-semibold text-slate-800 dark:text-zinc-100 uppercase tracking-wider text-xs flex items-center">
                <svg class="w-4 h-4 mr-2 text-slate-600 dark:text-zinc-400" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                Fast-Moving Items
              </h3>
              <span class="text-[10px] font-medium bg-slate-900 text-white dark:bg-zinc-100 dark:text-zinc-900 px-2.5 py-0.5 rounded-full">TOP 5</span>
            </div>
            <div class="p-4 space-y-2.5">
              <div v-for="item in dashboardData.product_intelligence?.fast_moving" :key="item.name" class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-950/60 rounded-xl border border-zinc-200/80 dark:border-zinc-800">
                <span class="text-xs font-medium text-slate-800 dark:text-zinc-200 truncate mr-2">{{ item.name }}</span>
                <span class="text-[11px] font-medium text-white bg-slate-900 dark:bg-zinc-100 dark:text-zinc-900 px-2.5 py-1 rounded-lg shadow-xs">{{ item.total_sold }} sold</span>
              </div>
          ...
          <div class="bg-white dark:bg-zinc-900 shadow-sm rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800">
            <div class="px-6 py-4 bg-zinc-100 dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
              <h3 class="font-semibold text-slate-800 dark:text-zinc-100 uppercase tracking-wider text-xs flex items-center">
                <svg class="w-4 h-4 mr-2 text-slate-600 dark:text-zinc-400" fill="currentColor" viewBox="0 0 20 20"><path d="M10 18a8 8 0 100-16 8 8 0 000-16zm1 11H9v-2h2v2zm0-4H9V7h2v10z"></path></svg>
                Slow-Moving Items
              </h3>
              <span class="text-[10px] font-medium bg-slate-900 text-white dark:bg-zinc-100 dark:text-zinc-900 px-2.5 py-0.5 rounded-full">IN STOCK</span>
            </div>
            <div class="p-4 space-y-2.5">
              <div v-for="item in dashboardData.product_intelligence?.slow_moving" :key="item.name" class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-950/60 rounded-xl border border-zinc-200/80 dark:border-zinc-800">
                <span class="text-xs font-medium text-slate-800 dark:text-zinc-200 truncate mr-2">{{ item.name }}</span>
                <span class="text-[11px] font-medium text-slate-800 bg-zinc-200 dark:bg-zinc-800 dark:text-white px-2.5 py-1 rounded-lg border border-zinc-300 dark:border-zinc-700">{{ item.stock_quantity }} left</span>
              </div>
          ...
          <div class="bg-white dark:bg-zinc-900 shadow-sm rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800">
            <div class="px-6 py-4 bg-zinc-100 dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
              ...
              <span class="text-[10px] font-medium bg-slate-900 text-white dark:bg-zinc-100 dark:text-zinc-900 px-2.5 py-0.5 rounded-full">{{ dashboardData.expiry_alerts?.count || 0 }} ALERTS</span>
            </div>
            <div class="p-4 space-y-2.5">
              <div v-for="item in dashboardData.expiry_alerts?.items" :key="item.name" class="p-3 bg-zinc-50 dark:bg-zinc-950/60 rounded-xl border border-zinc-200/80 dark:border-zinc-800 flex flex-col">
                <div class="flex justify-between items-center mb-1">
                  <span class="text-xs font-medium text-slate-800 dark:text-zinc-200 truncate mr-2">{{ item.name }}</span>
                  <span class="text-[10px] font-medium px-2 py-0.5 rounded-full uppercase bg-slate-900 text-white dark:bg-zinc-100 dark:text-zinc-900">{{ item.status }}</span>
                </div>
                <div class="flex justify-between text-[11px] font-medium text-zinc-500 italic">
                  <span>Expires: {{ item.expiry_date }}</span>
                  <span class="text-slate-800 dark:text-white font-semibold">{{ item.days_to_expire < 0 ? 'Expired' : item.days_to_expire + ' days' }}</span>
                </div>
              </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div class="lg:col-span-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-2xl overflow-hidden">
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
              <h3 class="text-sm font-bold text-zinc-950 dark:text-white uppercase tracking-wider">Recent Invoices</h3>
              <div class="flex items-center space-x-2">
                <div class="relative group">
                  <select class="pl-3 pr-8 py-1.5 bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 rounded-xl focus:border-slate-300 focus:ring-2 focus:ring-slate-100 outline-none dark:focus:border-zinc-600 transition-all font-semibold text-xs text-zinc-900 dark:text-white appearance-none cursor-pointer uppercase tracking-tight">
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
                <thead class="bg-zinc-100 dark:bg-zinc-800/80">
                  <tr>
                    <th class="px-6 py-3 text-left text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider">Invoice ID</th>
                    <th class="px-6 py-3 text-left text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider">Customer</th>
                    <th class="px-6 py-3 text-left text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider">Sales Date</th>
                    <th class="px-6 py-3 text-left text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider">Paid Amount</th>
                    <th class="px-6 py-3 text-left text-[11px] font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider">Sales Status</th>
                  </tr>
                </thead>
                <tbody class="bg-white dark:bg-zinc-900 divide-y divide-zinc-200 dark:divide-zinc-800">
                  <tr v-for="invoice in dashboardData.recent_invoices" :key="invoice.invoice_id" class="hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap text-xs font-semibold text-zinc-950 dark:text-white">
                      {{ invoice.invoice_id }}
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span class="inline-flex px-2.5 py-0.5 text-[10px] font-semibold rounded-full bg-black text-white dark:bg-white dark:text-black uppercase tracking-wider">
          ...
          <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-2xl overflow-hidden">
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
              <h3 class="text-sm font-bold text-zinc-950 dark:text-white uppercase tracking-wider">Stock History</h3>
              <div class="relative group">
                <select class="pl-3 pr-8 py-1.5 bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 rounded-xl focus:border-slate-300 focus:ring-2 focus:ring-slate-100 outline-none dark:focus:border-zinc-600 transition-all font-semibold text-xs text-zinc-900 dark:text-white appearance-none cursor-pointer uppercase tracking-tight">
            ...
            <div class="p-6 space-y-6">
              ...
              <div class="flex items-center justify-between pb-4 border-b border-zinc-100 dark:border-zinc-800">
                <div>
                  <p class="text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Total Sales Items</p>
                ...
                <div class="flex items-center">
                  <span class="text-xs font-semibold bg-zinc-100 text-zinc-900 dark:bg-zinc-800 dark:text-zinc-100 px-2.5 py-1 rounded-full border border-zinc-300 dark:border-zinc-700">
              ...
              <div class="flex items-center justify-between pb-4 border-b border-zinc-100 dark:border-zinc-800">
                <div>
                  <p class="text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Purchase Items</p>
              ...
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Purchase Returns Items</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-2xl overflow-hidden">
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
              <h3 class="text-sm font-bold text-zinc-950 dark:text-white uppercase tracking-wider">Payment Telemetry</h3>
              <div class="relative group">
                <select class="pl-3 pr-8 py-1.5 bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 rounded-xl focus:border-slate-300 focus:ring-2 focus:ring-slate-100 outline-none dark:focus:border-zinc-600 transition-all font-semibold text-xs text-zinc-900 dark:text-white appearance-none cursor-pointer uppercase tracking-tight">
            ...
            <div class="p-6">
              ...
              <div class="flex items-center justify-center space-x-8 mt-6">
                <div class="flex items-center">
                  ...
                  <span class="text-xs font-semibold text-zinc-700 dark:text-zinc-300">Payment Sent</span>
                </div>
                <div class="flex items-center">
                  <div class="w-3 h-3 bg-zinc-400 dark:bg-zinc-500 rounded-full mr-2"></div>
                  <span class="text-xs font-semibold text-zinc-700 dark:text-zinc-300">Payment Received</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-2xl overflow-hidden">
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800">
              <h3 class="text-sm font-bold text-zinc-950 dark:text-white uppercase tracking-wider">Stock Alert Thresholds</h3>
            </div>
            <div class="p-6">
              <div class="space-y-3 max-h-64 overflow-y-auto custom-scrollbar">
                <div v-for="alert in dashboardData.stock_alerts" :key="alert.product" class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-950/60 rounded-xl border border-zinc-200/80 dark:border-zinc-800">
                  <div>
                    <p class="text-xs font-semibold text-zinc-900 dark:text-zinc-100">{{ alert.product }}</p>
                  </div>
                  <div class="text-right">
                    <span class="text-xs font-semibold bg-black text-white dark:bg-white dark:text-black px-2.5 py-1 rounded-lg">
                      {{ alert.quantity }} LEFT
                    </span>
                  </div>
                </div>
        ...
        <div v-if="dashboardData.payments?.by_type?.length > 0" class="mt-8">
          <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-2xl overflow-hidden">
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
              <h3 class="text-sm font-bold text-zinc-950 dark:text-white uppercase tracking-wider">Payment Breakdown</h3>
              <router-link
                to="/payments"
                class="text-xs text-black dark:text-white font-semibold hover:underline"
              >
                View All →
              </router-link>
            </div>
            <div class="p-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <!-- By Type -->
                <div>
                  <h4 class="text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 mb-4">By Type</h4>
                  <div class="space-y-3">
                    <div v-for="type in dashboardData.payments.by_type" :key="type.payment_type" class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-950/60 rounded-xl border border-zinc-200/70 dark:border-zinc-800">
                      <span class="text-xs font-semibold text-zinc-800 dark:text-zinc-200">{{ getPaymentTypeDisplay(type.payment_type) }}</span>
                      <div class="text-right">
                        <span class="text-xs font-semibold text-zinc-950 dark:text-white">{{ formatAmount(type.total_amount) }}</span>
                ...
                <!-- By Status -->
                <div>
                  <h4 class="text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 mb-4">By Status</h4>
                  <div class="space-y-3">
                    <div v-for="status in dashboardData.payments.by_status" :key="status.status" class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-950/60 rounded-xl border border-zinc-200/70 dark:border-zinc-800">
                      <div class="flex items-center">
                        <span class="inline-flex px-2.5 py-0.5 text-[10px] font-semibold rounded-full uppercase tracking-wider bg-black text-white dark:bg-white dark:text-black">
                          {{ status.status }}
                        </span>
                      </div>
                      <div class="text-right">
                        <span class="text-xs font-semibold text-zinc-950 dark:text-white">{{ formatAmount(status.total_amount) }}</span>
