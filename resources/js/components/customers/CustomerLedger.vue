  ...
  <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" v-if="show">
    <div class="relative top-20 mx-auto p-5 border w-11/12 max-w-6xl shadow-lg rounded-md bg-white">
      ...
      <div class="flex justify-between items-center mb-6">
        <div>
          <h3 class="text-2xl font-bold text-gray-900">Customer Ledger</h3>
...
const loadLedgerData = async () => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/customers/${props.customer.id}/ledger`, {
      params: {
...
const exportLedger = () => {
  ...
  console.log('Export ledger functionality to be implemented');
};
