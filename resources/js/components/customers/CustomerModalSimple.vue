  ...
  setup(props, { emit }) {
    ...
    const loadCustomerData = () => {
      if (props.customer && props.isEdit) {
        ...
        if (props.customer.profile_image) {
          photoPreview.value = props.customer.profile_image.startsWith('http') ? props.customer.profile_image : `/storage/${props.customer.profile_image}`;
        }

        if (props.customer.attachments_urls && Array.isArray(props.customer.attachments_urls)) {
          existingAttachments.value = [...props.customer.attachments_urls];
        } else {
          existingAttachments.value = [];
        }
      }
    ...
    const saveCustomer = async () => {
      ...
      try {
        ...
        formData.set('type', form.type || 'registered');
        ...
        if (props.isEdit) {
          ...
          response = await api.post(`/customers/${props.customer.id}`, formData, {
