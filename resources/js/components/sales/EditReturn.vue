              ...
              <tbody class="divide-y divide-slate-100 dark:divide-zinc-800">
                ...
                <tr v-for="(item, idx) in form.items" :key="idx" class="hover:bg-slate-50/50 dark:hover:bg-zinc-800/30">
                  <td class="py-3 px-3">
                    ...
                    <div class="text-[10px] text-slate-400 mt-0.5" v-if="item.original_qty">Original Sold Qty: {{ item.original_qty }}</div>
                  </td>
...
const addProductToReturn = (product) => {
  ...
  const varId = product.product_variation_id || product.variation_id || null
  const existing = form.items.find(i => (i.product_id === prodId && i.product_variation_id === varId) || i.product_id === prodId)
  if (existing) {
    existing.quantity += 1
  ...
  try {
    ...
    form.items = rawItems.map(item => {
      ...
      return {
        ...
        brand_name: item.product?.brand_name || (typeof item.product?.brand === 'string' ? item.product?.brand : item.product?.brand?.name),
        category_path: item.product?.category_path || (typeof item.product?.category === 'string' ? item.product?.category : item.product?.category?.name),
