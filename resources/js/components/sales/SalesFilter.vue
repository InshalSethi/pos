    ...
    >
      ...
      >
        ...
        <div class="px-6 py-4 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between bg-slate-50/50 dark:bg-zinc-900/50">
          <div>
            <h2 class="text-base font-bold text-slate-900 dark:text-zinc-100 tracking-tight">Filter</h2>
          </div>
          <button
            @click="close"
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar" @click="closeAllPopovers">
          ...
          <div v-if="activeFilterCount > 0" class="flex items-center justify-between px-3 py-2 bg-slate-100 dark:bg-zinc-800/80 border border-slate-200 dark:border-zinc-700 rounded-lg text-xs">
            <span class="text-slate-800 dark:text-zinc-200 font-medium">
              <strong class="font-bold">{{ activeFilterCount }}</strong> active filter{{ activeFilterCount > 1 ? 's' : '' }} applied
            </span>
            <button
              @click="resetFilters"
              class="text-slate-600 dark:text-zinc-400 hover:underline font-semibold cursor-pointer"
            >
              Reset All
            </button>
          </div>
          ...
          <div class="space-y-1.5 relative" @click.stop>
            ...
            <div class="relative">
              <button
                ...
                class="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-zinc-800/80 border border-slate-200 dark:border-zinc-700 rounded-lg text-slate-800 dark:text-zinc-100 flex items-center justify-between focus:outline-none focus:border-slate-300 focus:ring-2 focus:ring-slate-100 dark:focus:border-zinc-600 dark:focus:ring-zinc-800 transition-all cursor-pointer"
                :class="{ 'border-slate-300 dark:border-zinc-600 ring-2 ring-slate-100 dark:ring-zinc-800 bg-white dark:bg-zinc-800': activePopover === 'date_range' }"
              >
              ...
              >
                ...
                <div class="flex flex-wrap gap-1 pb-1.5 mb-1.5 border-b border-slate-100 dark:border-zinc-800">
                  <button
                    type="button"
                    @click="setQuickDate('today')"
                    class="px-1.5 py-0.5 text-[10px] font-medium bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 hover:text-slate-900 dark:hover:bg-zinc-700 text-slate-700 dark:text-zinc-200 rounded transition-colors cursor-pointer"
                  >
                    Today
                  </button>
                  <button
                ...
                <div class="flex items-center justify-between pb-1 mb-0.5 px-0.5 relative">
                  ...
                  <div class="flex items-center space-x-1 relative">
                    ...
                    <button
                      ...
                      :class="{ 'bg-slate-100 dark:bg-zinc-800 text-slate-900 dark:text-zinc-100': subPickerOpen === 'month' }"
                    >
                    ...
                    <button
                      ...
                      class="px-2 py-0.5 font-bold text-slate-800 dark:text-zinc-100 text-xs hover:bg-slate-100 dark:hover:bg-zinc-800 rounded transition-colors flex items-center gap-1 cursor-pointer"
                      :class="{ 'bg-slate-100 dark:bg-zinc-800 text-slate-900 dark:text-zinc-100': subPickerOpen === 'year' }"
                    >
                  ...
                  >
                    <button
                      ...
                      :class="calendarMonth === mIdx ? 'bg-slate-900 text-white font-bold shadow-xs' : 'hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-700 dark:text-zinc-200'"
                    >
                      {{ mName }}
                    </button>
                  </div>
                  ...
                  >
                    <button
                      ...
                      class="py-1 text-xs font-semibold rounded text-center transition-colors cursor-pointer"
                      :class="calendarYear === y ? 'bg-slate-900 text-white font-bold shadow-xs' : 'hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-700 dark:text-zinc-200'"
                    >
                      {{ y }}
                    </button>
                  </div>
                ...
                <div class="grid grid-cols-7 gap-y-0.5 text-center" @mouseleave="hoverDate = null">
                  ...
                  >
                    ...
                    <div
                      ...
                      class="absolute inset-y-0.5 inset-x-0 bg-slate-100 dark:bg-zinc-800"
                      :class="{
                    ...
                    <button
                      ...
                      :class="{
                        ...
                        'bg-slate-900 text-white font-bold shadow-xs': isDateStart(day.dateStr) || isDateEnd(day.dateStr),
                        'text-slate-900 dark:text-zinc-100 font-bold': isDateInRange(day.dateStr) && !isDateStart(day.dateStr) && !isDateEnd(day.dateStr),
                        ...
                        'ring-1 ring-slate-400': isToday(day.dateStr) && !isDateStart(day.dateStr) && !isDateEnd(day.dateStr)
                      }"
                    >
                      {{ day.dayNum }}
                    </button>
                ...
                <div class="mt-1.5 pt-1.5 border-t border-slate-100 dark:border-zinc-800 flex items-center justify-between text-[10px] text-slate-500 dark:text-zinc-400">
                  <div>
                    <span v-if="localFilters.date_from && !localFilters.date_to" class="text-slate-700 dark:text-zinc-300 font-medium">
                      Select end date...
                    </span>
                  ...
                  <button
                    type="button"
                    @click="togglePopover('date_range')"
                    class="px-2 py-0.5 font-bold text-white bg-slate-900 hover:bg-black rounded transition-colors text-[10px] cursor-pointer"
                  >
                    Done
                  </button>
                </div>
          ...
          <div class="space-y-1.5 relative" @click.stop>
            ...
            <div class="relative">
              <button
                type="button"
                @click="togglePopover('customer')"
                class="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-zinc-800/80 border border-slate-200 dark:border-zinc-700 rounded-lg text-slate-800 dark:text-zinc-100 flex items-center justify-between focus:outline-none focus:border-slate-300 focus:ring-2 focus:ring-slate-100 dark:focus:border-zinc-600 dark:focus:ring-zinc-800 transition-all cursor-pointer"
                :class="{ 'border-slate-300 dark:border-zinc-600 ring-2 ring-slate-100 dark:ring-zinc-800 bg-white dark:bg-zinc-800': activePopover === 'customer' }"
              >
                <span class="truncate pr-2" :class="{ 'text-slate-400 dark:text-zinc-500': localFilters.customer_ids.length === 0 }">
                  {{ customerSummaryLabel }}
                </span>
                <span v-if="localFilters.customer_ids.length > 0" class="text-[10px] font-bold bg-slate-200 text-slate-800 dark:bg-zinc-700 dark:text-zinc-200 px-1.5 py-0.2 rounded-full shrink-0">
                  {{ localFilters.customer_ids.length }}
                </span>
              </button>
          ...
          <div class="space-y-1.5 relative" @click.stop>
            ...
            <div class="relative">
              <button
                ...
                class="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-zinc-800/80 border border-slate-200 dark:border-zinc-700 rounded-lg text-slate-800 dark:text-zinc-100 flex items-center justify-between focus:outline-none focus:border-slate-300 focus:ring-2 focus:ring-slate-100 dark:focus:border-zinc-600 dark:focus:ring-zinc-800 transition-all cursor-pointer"
                :class="{ 'border-slate-300 dark:border-zinc-600 ring-2 ring-slate-100 dark:ring-zinc-800 bg-white dark:bg-zinc-800': activePopover === 'counter' }"
              >
                <span class="truncate pr-2" :class="{ 'text-slate-400 dark:text-zinc-500': localFilters.counter_ids.length === 0 }">
                  {{ counterSummaryLabel }}
                </span>
                <span v-if="localFilters.counter_ids.length > 0" class="text-[10px] font-bold bg-slate-200 text-slate-800 dark:bg-zinc-700 dark:text-zinc-200 px-1.5 py-0.2 rounded-full shrink-0">
...
const availableStatuses = [
  ...
  { id: 'due', label: 'Due', colorClass: 'text-orange-600 dark:text-orange-400' },
...
const customerSummaryLabel = computed(() => {
  const ids = localFilters.value.customer_ids || [];
  if (ids.length === 0) return 'All Customers / Clients';
  if (ids.length === 1) {
    const c = customers.value.find(item => String(item.id) === String(ids[0]));
    return c ? (c.name || c.full_name) : `Customer #${ids[0]}`;
  }
  const firstC = customers.value.find(item => String(item.id) === String(ids[0]));
  const firstName = firstC ? (firstC.name || firstC.full_name) : `Customer #${ids[0]}`;
...
const syncPropsToLocal = () => {
  ...
  if (localFilters.value.date_from) {
    const parts = localFilters.value.date_from.split('-');
    if (parts.length === 3) {
      calendarYear.value = parseInt(parts[0]);
      calendarMonth.value = parseInt(parts[1]) - 1;
    }
