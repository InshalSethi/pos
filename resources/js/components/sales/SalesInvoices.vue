<template>
  <div class="w-full max-w-full py-8 px-4 sm:px-6 lg:px-8">
    ...
    <!-- Tabs Bar -->
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200 mb-6 pb-0.5 space-y-4 sm:space-y-0">
      <div class="flex flex-wrap gap-x-6 gap-y-2">
        <button
          ...
          :class="isTabActive(tab.id) ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'"
        >
          <span>{{ tab.label }}</span>
          <span
            class="text-[10px] px-1.5 py-0.5 rounded-full font-bold"
            :class="isTabActive(tab.id) ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-600'"
          >
            {{ counts[tab.id] || 0 }}
          </span>
        </button>
      ...
      <div class="flex items-center space-x-2">
        <!-- Sort Button -->
        <button class="inline-flex items-center px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-semibold text-slate-600 bg-white hover:bg-slate-55 shadow-sm transition-all">
        ...
        <div class="relative">
          <button
            ...
            class="inline-flex items-center px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-semibold text-slate-600 bg-white hover:bg-slate-50 shadow-sm transition-all focus:outline-none cursor-pointer"
          ...
          <div
            ...
            class="absolute right-0 mt-1 w-36 bg-white border border-slate-200 rounded-lg shadow-lg py-1.5 z-50 animate-fade-in"
          >
            <button
              v-for="option in ['draft', 'paid', 'due', 'recurring', 'overdue']"
              :key="option"
              @click.stop="toggleFilterOption(option)"
              class="w-full text-left px-3 py-1.5 text-xs hover:bg-slate-50 flex items-center justify-between transition-colors"
              :class="selectedFilters.includes(option) ? 'text-blue-600 font-bold bg-blue-50/20' : 'text-slate-700'"
            >
        ...
        <button
          ...
          class="inline-flex items-center px-3 py-1.5 border border-rose-200 text-rose-600 hover:text-rose-700 hover:bg-rose-50 rounded-lg text-xs font-semibold shadow-sm transition-all focus:outline-none cursor-pointer"
        >
    ...
    <div class="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-soft">
      <div class="flex items-center justify-between p-4 border-b border-slate-100">
        ...
        <div class="relative w-64">
          ...
          <input
            ...
            class="w-full pl-9 pr-4 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all text-slate-700"
        ...
        <div class="flex items-center space-x-2 text-xs text-slate-500">
          ...
          <select
            ...
            class="border border-slate-200 rounded px-1.5 py-0.5 focus:outline-none focus:ring-1 focus:ring-blue-500 cursor-pointer bg-white"
          >
      ...
      <div class="overflow-x-auto custom-scrollbar">
        <table class="w-full text-left text-xs border-collapse">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase font-bold tracking-wider">
              <th class="py-3.5 px-4 w-[40px] text-center bg-slate-50">
                <input type="checkbox" class="rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer w-3.5 h-3.5" />
              </th>
              <th class="py-3.5 px-4 cursor-pointer hover:bg-slate-100/50 bg-slate-50" @click="handleSort('sale_number')">
              ...
              <th class="py-3.5 px-4 bg-slate-50">Client/Customer</th>
              <th class="py-3.5 px-4 text-right cursor-pointer hover:bg-slate-100/50 bg-slate-50" @click="handleSort('total_amount')">
                <div class="flex items-center justify-end space-x-1">
                  <span>Total Amount</span>
                  <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4"/></svg>
                </div>
              </th>
              <th class="py-3.5 px-4 text-right bg-slate-50">Paid Amount</th>
              <th class="py-3.5 px-4 text-right bg-slate-50">Due Amount</th>
              <th class="py-3.5 px-4 cursor-pointer hover:bg-slate-100/50 bg-slate-50" @click="handleSort('due_date')">
                <div class="flex items-center space-x-1">
                  <span>Due Date</span>
                  <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4"/></svg>
                </div>
              </th>
              <th class="py-3.5 px-4 text-center cursor-pointer hover:bg-slate-100/50 bg-slate-50" @click="handleSort('status')">
                <div class="flex items-center justify-center space-x-1">
                  <span>Status</span>
                  <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4"/></svg>
                </div>
              </th>
              <th class="py-3.5 px-4 text-center bg-slate-50 w-[80px]">Action</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100/70">
            <tr v-if="loading" class="bg-white">
              <td colspan="9" class="py-12 text-center text-slate-400">
                <div class="flex flex-col items-center justify-center space-y-2">
                  <div class="animate-spin rounded-full h-7 w-7 border-2 border-slate-300 border-t-blue-600"></div>
                  <span class="text-xs font-semibold">Loading invoices...</span>
                </div>
              </td>
            </tr>
            <tr v-else-if="invoices.length === 0" class="bg-white">
              <td colspan="9" class="py-16 text-center text-slate-450 italic">
            ...
            <tr v-else v-for="item in invoices" :key="item.id" class="hover:bg-slate-50/50 transition-colors">
              <!-- Checkbox -->
              <td class="py-4 px-4 text-center align-middle bg-white">
                <input type="checkbox" class="rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer w-3.5 h-3.5" />
              </td>

              <!-- Invoice ID -->
              <td class="py-4 px-4 align-middle bg-white">
                <div class="font-bold text-slate-800 text-sm hover:text-blue-600 cursor-pointer" @click="viewInvoice(item)">
                  {{ item.sale_number }}
                </div>
              ...
              <td class="py-4 px-4 align-middle bg-white">
                ...
                <div class="text-[10px] text-slate-400 flex items-center mt-0.5 space-x-1">
              ...
              <!-- Total Amount -->
              <td class="py-4 px-4 text-right font-bold text-slate-850 text-sm align-middle bg-white">
                {{ formatCurrency(item.total_amount) }}
              </td>

              <!-- Paid Amount -->
              <td class="py-4 px-4 text-right font-semibold text-emerald-600 text-sm align-middle bg-white">
                {{ parseFloat(item.paid_amount) > 0 ? formatCurrency(item.paid_amount) : '-' }}
              </td>

              <!-- Due Amount -->
              <td class="py-4 px-4 text-right font-semibold text-rose-500 text-sm align-middle bg-white">
                {{ getDueAmount(item) > 0 ? formatCurrency(getDueAmount(item)) : '-' }}
              </td>

              <!-- Due Date -->
              <td class="py-4 px-4 text-slate-600 text-sm align-middle bg-white">
              ...
              <td class="py-4 px-4 text-center relative align-middle bg-white">
                <button
                  @click.stop="toggleActionDropdown(item.id)"
                  class="text-slate-400 hover:text-slate-600 p-1 rounded-full hover:bg-slate-100 transition-all focus:outline-none"
                >
                ...
                <div
                  ...
                  class="absolute right-4 mt-1 w-32 bg-white border border-slate-200 rounded-lg shadow-lg py-1 z-50 animate-fade-in"
                >
                  <button @click="viewInvoice(item)" class="w-full text-left px-3 py-1.5 text-xs text-slate-700 hover:bg-slate-50 flex items-center space-x-1.5">
                    <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                    <span>View</span>
                  </button>
                  <button @click="editInvoice(item)" class="w-full text-left px-3 py-1.5 text-xs text-slate-700 hover:bg-slate-50 flex items-center space-x-1.5">
                  ...
                  <button @click="printInvoice(item)" class="w-full text-left px-3 py-1.5 text-xs text-slate-700 hover:bg-slate-50 flex items-center space-x-1.5">
                    <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                    <span>Print</span>
                  </button>
                  <div class="border-t border-slate-100 my-1"></div>
                  <button @click="deleteInvoice(item.id)" class="w-full text-left px-3 py-1.5 text-xs text-rose-600 hover:bg-rose-50 flex items-center space-x-1.5">
      ...
      <div class="flex items-center justify-between p-4 border-t border-slate-100 bg-white">
        <div class="flex-1 flex justify-between sm:hidden">
          <button
            ...
            class="relative inline-flex items-center px-4 py-2 border border-slate-200 text-xs font-semibold rounded-lg text-slate-700 bg-white hover:bg-slate-55 disabled:opacity-50"
          >
            Previous
          </button>
          <button
            ...
            class="ml-3 relative inline-flex items-center px-4 py-2 border border-slate-200 text-xs font-semibold rounded-lg text-slate-700 bg-white hover:bg-slate-55 disabled:opacity-50"
          >
            Next
          </button>
        </div>
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-center">
          <nav class="relative z-0 inline-flex rounded-lg shadow-sm -space-x-px" aria-label="Pagination">
            ...
            <button
              @click="previousPage"
              :disabled="currentPage === 1"
              class="relative inline-flex items-center px-2.5 py-2 rounded-l-lg border border-slate-200 bg-white text-xs font-semibold text-slate-500 hover:bg-slate-50 disabled:opacity-50 cursor-pointer"
            >
            ...
            <template v-for="(page, idx) in paginationRange" :key="idx">
              <span
                v-if="page === '...'"
                class="relative inline-flex items-center px-3.5 py-2 border border-slate-200 bg-white text-xs font-bold text-slate-400 select-none"
              >
                ...
              </span>
              <button
                ...
                :class="currentPage === page ? 'z-10 bg-slate-105 border-slate-300 text-slate-800' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'"
              >
                {{ page }}
              </button>
            </template>
            ...
            <button
              @click="nextPage"
              :disabled="currentPage === pagination.last_page"
              class="relative inline-flex items-center px-2.5 py-2 rounded-r-lg border border-slate-200 bg-white text-xs font-semibold text-slate-500 hover:bg-slate-50 disabled:opacity-50 cursor-pointer"
            >
...
const getStatusBadgeClass = (item) => {
  ...
  if (label === 'Due') return 'bg-orange-50 text-orange-700';
  if (label === 'Overdue') return 'bg-rose-50 text-rose-700';
  if (label === 'Draft') return 'bg-slate-100 text-slate-650';
