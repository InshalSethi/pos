<template>
  <div class="space-y-8 font-sans">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <!-- FBR PAKISTAN CARD -->
      <div
        v-if="activeCategory === 'all' || activeCategory === 'pakistan'"
      ...
      >
        <div class="space-y-4">
          <div class="flex items-start justify-between">
            <div class="flex items-center space-x-3">
              ...
              <div>
                ...
                <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Federal Board of Revenue</span>
              </div>
            </div>

            <!-- Status Badge -->
            <span v-if="fbrSetting.is_enabled" class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800">
          ...
          <p class="text-xs text-slate-600 dark:text-slate-400 line-clamp-2">
            Automated POS fiscalization & digital invoicing for Sales, Purchases, Transactions, and Payments with FBR IRIS verification.
          </p>

          <div v-if="fbrSetting.is_enabled" class="pt-2 flex flex-wrap gap-2 text-[10px] font-semibold text-slate-500 dark:text-slate-400">
            <span class="bg-slate-100 dark:bg-zinc-800 px-2 py-0.5 rounded-md">POSID: {{ fbrSetting.pos_id || 'N/A' }}</span>
            <span class="bg-slate-100 dark:bg-zinc-800 px-2 py-0.5 rounded-md capitalize">Mode: {{ fbrSetting.environment }}</span>
          </div>
        </div>
          ...
          <div class="flex items-center space-x-2">
            <button
              ...
              :class="[
                ...
                fbrSetting.is_enabled ? 'bg-emerald-600' : 'bg-slate-300 dark:bg-zinc-700'
              ]"
            >
              <span
                :class="[
                  'pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-xs ring-0 transition duration-200 ease-in-out',
                  fbrSetting.is_enabled ? 'translate-x-5' : 'translate-x-0'
                ]"
              ></span>
            </button>
            <span class="text-[11px] font-bold text-slate-600 dark:text-slate-400">
              {{ fbrSetting.is_enabled ? 'Active' : 'Off' }}
            </span>
          </div>
      ...
      <div
        v-if="activeCategory === 'all' || activeCategory === 'pakistan'"
        class="bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 rounded-2xl p-5 shadow-xs flex flex-col justify-between opacity-80"
      >
        <div class="space-y-4">
          <div class="flex items-start justify-between">
            <div class="flex items-center space-x-3">
              <div class="w-12 h-12 rounded-xl bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 flex items-center justify-center font-black text-xs border border-amber-300/40">
                PRA
              </div>
              <div>
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">PRA Punjab</h3>
                <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Punjab Revenue Authority</span>
              </div>
            </div>
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400">
            Services Sales Tax (PST) digital invoicing integration for restaurants, hotels, and service sector compliance in Punjab.
          </p>
        </div>
      ...
      >
        <div class="space-y-4">
          <div class="flex items-start justify-between">
            <div class="flex items-center space-x-3">
              <div class="w-12 h-12 rounded-xl bg-blue-100 dark:bg-blue-950/60 text-blue-700 dark:text-blue-400 flex items-center justify-center font-black text-xs border border-blue-300/40">
                SRB
              </div>
              <div>
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">SRB Sindh</h3>
                <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Sindh Revenue Board</span>
              </div>
            </div>
            <span class="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-indigo-50 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
              Coming Soon
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400">
            Sindh Sales Tax on Services (SST) real-time fiscal record transmission and automated verification portal.
          </p>
        </div>
        <div class="mt-6 pt-4 border-t border-slate-100 dark:border-zinc-800 flex justify-end">
          <button disabled class="px-3.5 py-1.5 bg-slate-100 text-slate-400 dark:bg-zinc-800 dark:text-zinc-500 text-xs font-bold rounded-xl cursor-not-allowed">
      ...
      >
        <div class="space-y-4">
          <div class="flex items-start justify-between">
            <div class="flex items-center space-x-3">
              <div class="w-12 h-12 rounded-xl bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 flex items-center justify-center font-black text-xs border border-emerald-300/40">
                KPRA
              </div>
              <div>
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">KPRA KP</h3>
                <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Khyber Pakhtunkhwa Revenue Authority</span>
              </div>
            </div>
            <span class="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-indigo-50 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
              Coming Soon
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400">
            KP digital tax portal integration for automated transaction logging and service tax invoices.
          </p>
        </div>
        <div class="mt-6 pt-4 border-t border-slate-100 dark:border-zinc-800 flex justify-end">
          <button disabled class="px-3.5 py-1.5 bg-slate-100 text-slate-400 dark:bg-zinc-800 dark:text-zinc-500 text-xs font-bold rounded-xl cursor-not-allowed">
      ...
      >
        <div class="space-y-4">
          <div class="flex items-start justify-between">
            <div class="flex items-center space-x-3">
              <div class="w-12 h-12 rounded-xl bg-teal-100 dark:bg-teal-950/60 text-teal-700 dark:text-teal-400 flex items-center justify-center font-black text-xs border border-teal-300/40">
                BRA
              </div>
              <div>
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">BRA Balochistan</h3>
                <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Balochistan Revenue Authority</span>
              </div>
            </div>
            <span class="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-indigo-50 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
              Coming Soon
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400">
            Balochistan sales tax compliance and automated fiscal record reporting integration.
          </p>
        </div>
        <div class="mt-6 pt-4 border-t border-slate-100 dark:border-zinc-800 flex justify-end">
          <button disabled class="px-3.5 py-1.5 bg-slate-100 text-slate-400 dark:bg-zinc-800 dark:text-zinc-500 text-xs font-bold rounded-xl cursor-not-allowed">
      ...
      <div
        v-if="activeCategory === 'all' || activeCategory === 'international'"
        class="bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 rounded-2xl p-5 shadow-xs flex flex-col justify-between opacity-80"
      >
        <div class="space-y-4">
          <div class="flex items-start justify-between">
    ...
    <!-- ==================== FBR CONFIGURATION & AUDIT POPUP MODAL ==================== -->
    <Teleport to="body" v-if="showFbrModal">
      <div class="fixed inset-0 z-[99999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
        ...
        <div class="flex items-center justify-between border-b border-slate-200 dark:border-zinc-800 pb-4">
          <div class="flex items-center space-x-3">
            <div class="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 flex items-center justify-center font-black text-base border border-emerald-300/40">
              🇵🇰
            </div>
            <div>
              <h3 class="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                FBR Pakistan Integration Configuration
              </h3>
              <p class="text-xs text-slate-500 dark:text-slate-400">
                Configure credentials, transaction scopes, and audit fiscalized logs for company ID: {{ selectedCompanyId }}
              </p>
            </div>
          </div>

          <button
            @click="showFbrModal = false"
            class="w-8 h-8 rounded-full bg-slate-100 dark:bg-zinc-800 text-slate-400 hover:text-slate-700 dark:hover:text-white flex items-center justify-center font-bold text-lg"
          >
            &times;
          </button>
        </div>
        ...
        <div class="flex items-center space-x-2 border-b border-slate-100 dark:border-zinc-800 pb-2">
          <button
            @click="fbrModalTab = 'settings'"
            :class="[
              'px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer',
              fbrModalTab === 'settings'
                ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900 shadow-xs'
                : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-zinc-800'
            ]"
          >
        ...
        <div v-if="fbrModalTab === 'settings'" class="space-y-6">
          ...
          <div class="bg-slate-50 dark:bg-zinc-800/50 border border-slate-200/80 dark:border-zinc-700/60 rounded-2xl p-5 flex items-center justify-between">
            <div class="flex items-center space-x-4">
              <div :class="[
                ...
                fbrSetting.is_enabled ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-400' : 'bg-slate-200 text-slate-500 dark:bg-zinc-700 dark:text-zinc-400'
              ]">
              ...
              <div>
                <h4 class="text-sm font-bold text-slate-900 dark:text-white">Enable FBR Pakistan Fiscalization</h4>
                <p class="text-xs text-slate-500 dark:text-slate-400">
                  When enabled, sales, purchases, transactions, and payments for this company will record to FBR.
                </p>
              </div>
            </div>

            <button
              @click="fbrSetting.is_enabled = !fbrSetting.is_enabled"
              :class="[
            ...
            >
              <span
                :class="[
                  ...
                  fbrSetting.is_enabled ? 'translate-x-7' : 'translate-x-0'
                ]"
              ></span>
            </button>
          </div>
          ...
          <!-- Scope Toggles -->
          <div v-if="fbrSetting.is_enabled" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="p-3 bg-white dark:bg-zinc-800 rounded-xl border border-slate-200 dark:border-zinc-700 flex items-center justify-between">
              ...
              <input type="checkbox" v-model="fbrSetting.sync_sales" class="w-4 h-4 text-emerald-600 rounded cursor-pointer" />
            </div>
            ...
            <div class="p-3 bg-white dark:bg-zinc-800 rounded-xl border border-slate-200 dark:border-zinc-700 flex items-center justify-between">
              ...
              <input type="checkbox" v-model="fbrSetting.sync_purchases" class="w-4 h-4 text-emerald-600 rounded cursor-pointer" />
            </div>
            ...
            <div class="p-3 bg-white dark:bg-zinc-800 rounded-xl border border-slate-200 dark:border-zinc-700 flex items-center justify-between">
              ...
              <input type="checkbox" v-model="fbrSetting.sync_transactions" class="w-4 h-4 text-emerald-600 rounded cursor-pointer" />
            </div>
            ...
            <div class="p-3 bg-white dark:bg-zinc-800 rounded-xl border border-slate-200 dark:border-zinc-700 flex items-center justify-between">
              ...
              <input type="checkbox" v-model="fbrSetting.sync_payments" class="w-4 h-4 text-emerald-600 rounded cursor-pointer" />
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <select
                  v-model="fbrSetting.environment"
              ...
              <div>
                ...
                <input
                  type="text"
                  v-model="fbrSetting.business_name"
              ...
              <div>
                ...
                <input
                  type="text"
                  v-model="fbrSetting.branch_name"
          ...
          <div class="pt-4 border-t border-slate-200 dark:border-zinc-800 flex items-center justify-between">
            <div class="flex items-center space-x-2">
              <input type="checkbox" id="auto_sync" v-model="fbrSetting.auto_sync" class="w-4 h-4 text-emerald-600 rounded cursor-pointer" />
              <label for="auto_sync" class="text-xs font-bold text-slate-700 dark:text-slate-300 cursor-pointer">
                Automatically post entries to FBR on transaction creation
              </label>
            </div>

            <button
              @click="saveFbrSettings"
              :disabled="saving"
            ...
            >
              ...
              <span>{{ saving ? 'Saving FBR Settings...' : 'Save FBR Settings' }}</span>
            </button>
          </div>
        </div>

        <!-- TAB 2: FISCAL AUDIT LOGS -->
        <div v-if="fbrModalTab === 'logs'" class="space-y-4">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div class="p-3 bg-emerald-50/60 dark:bg-emerald-950/20 rounded-xl border border-emerald-200/60 dark:border-emerald-800/40">
              <span class="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider block">Synced to FBR</span>
            ...
            <table class="w-full text-left text-xs">
              <thead class="bg-slate-50 dark:bg-zinc-800 text-slate-600 dark:text-slate-300 font-bold uppercase tracking-wider">
                <tr>
                  ...
                  <th class="px-3 py-2.5">FBR Invoice IRN</th>
                ...
                <tr v-for="e in entries.data" :key="e.id" class="hover:bg-slate-50/50 dark:hover:bg-zinc-800/40 font-medium">
                  ...
                  <td class="px-3 py-2.5 text-right space-x-1">
                    <button
                      ...
                      class="px-2 py-1 bg-slate-100 hover:bg-slate-200 dark:bg-zinc-800 text-slate-700 dark:text-slate-200 font-semibold text-[11px] rounded-lg"
                    >
                      Details
                    </button>
                    <button
                      ...
                      :disabled="e.syncing"
                      class="px-2 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 font-semibold text-[11px] rounded-lg disabled:opacity-50"
                    >
    ...
    <Teleport to="body" v-if="selectedEntry">
      ...
      <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl max-w-xl w-full p-5 space-y-4 shadow-xl max-h-[85vh] overflow-y-auto">
        <div class="flex items-center justify-between border-b border-slate-200 dark:border-zinc-800 pb-2">
          ...
          <button @click="selectedEntry = null" class="text-slate-400 hover:text-slate-600 font-bold">&times;</button>
        </div>

        <div class="grid grid-cols-2 gap-3 text-xs">
          <div>
            <span class="text-slate-400 block font-medium">FBR IRN</span>
        ...
        <div class="flex justify-end pt-2">
          <button @click="selectedEntry = null" class="px-4 py-1.5 bg-slate-900 text-white dark:bg-white dark:text-slate-900 text-xs font-bold rounded-xl">
            Close
          </button>
        </div>
      </div>
...
const showFbrModal = ref(false);
const fbrModalTab = ref('settings');
...
const loadCompanySettings = async () => {
  try {
    ...
    if (res.data?.setting) {
      fbrSetting.value = { ...fbrSetting.value, ...res.data.setting };
    }
...
const openFbrModal = () => {
  showFbrModal.value = true;
};

const toggleFbrStatus = async () => {
  fbrSetting.value.is_enabled = !fbrSetting.value.is_enabled;
  await saveFbrSettings();
};

const saveFbrSettings = async () => {
  saving.value = true;
  try {
    const payload = {
      ...fbrSetting.value,
...
const testConnection = async () => {
  ...
  try {
    const res = await axios.post('/api/fbr-settings/test-connection', {
      company_id: selectedCompanyId.value,
      pos_id: fbrSetting.value.pos_id,
      environment: fbrSetting.value.environment,
      api_token: fbrSetting.value.api_token,
      base_url: fbrSetting.value.base_url,
    });
