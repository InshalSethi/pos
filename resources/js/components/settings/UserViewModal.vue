    ...
    <div class="relative mx-auto border border-slate-200 dark:border-zinc-800 w-full max-w-4xl shadow-2xl rounded-2xl bg-white dark:bg-zinc-900 text-slate-800 dark:text-slate-100 overflow-hidden transition-all duration-300 z-10 max-h-[90vh] overflow-y-auto my-auto">
        <div class="bg-white dark:bg-zinc-900">
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center space-x-4">
                <div class="flex-shrink-0">
                  <div class="w-12 h-12 rounded-xl overflow-hidden bg-white/10 flex items-center justify-center border border-white/10 shadow-sm">
                    <img v-if="userData?.avatar_url || userData?.profile_image" :src="userData.avatar_url || (userData.profile_image.startsWith('http') ? userData.profile_image : `/storage/${userData.profile_image}`)" class="w-full h-full object-cover" />
                ...
                <div v-if="userData">
                  <h3 class="text-lg font-bold text-white">{{ userData.name }}</h3>
                  <p class="text-slate-400 text-xs font-medium">ID: #{{ userData.id }}</p>
                </div>
              </div>
              <div class="flex items-center space-x-3">
                <span v-if="userData" :class="userData.is_active 
                  ? 'px-3 py-1 text-xs font-bold rounded-full bg-slate-100 text-slate-900 dark:bg-zinc-100 dark:text-zinc-900 uppercase' 
                  : 'px-3 py-1 text-xs font-bold rounded-full bg-slate-800 text-slate-400 uppercase'">
                  {{ userData.is_active ? 'ACTIVE' : 'INACTIVE' }}
                </span>
                <button @click="closeModal" class="text-slate-400 hover:text-white p-1 rounded-lg hover:bg-white/10 cursor-pointer">
          ...
          <div v-if="loading" class="py-20 text-center">
            <div class="animate-spin inline-block w-8 h-8 border-4 border-slate-900 dark:border-white border-t-transparent rounded-full mb-2"></div>
            <p class="text-xs font-semibold text-slate-500 dark:text-zinc-400">Fetching details...</p>
          </div>

          <div v-else-if="userData" class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              <div class="p-4 bg-slate-50/70 dark:bg-zinc-800/40 border border-slate-200/80 dark:border-zinc-800 rounded-2xl">
                <p class="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Email</p>
                <p class="text-xs font-bold text-slate-900 dark:text-zinc-100 truncate mt-1">{{ userData.email }}</p>
              </div>
              <div class="p-4 bg-slate-50/70 dark:bg-zinc-800/40 border border-slate-200/80 dark:border-zinc-800 rounded-2xl">
                <p class="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Role</p>
                <p class="text-xs font-bold text-slate-900 dark:text-zinc-100 capitalize mt-1">{{ userData.roles?.[0]?.name || 'No Role' }}</p>
              </div>
              <div class="p-4 bg-slate-50/70 dark:bg-zinc-800/40 border border-slate-200/80 dark:border-zinc-800 rounded-2xl">
                <p class="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Registered On</p>
                <p class="text-xs font-bold text-slate-900 dark:text-zinc-100 mt-1">{{ formatDate(userData.created_at) }}</p>
              </div>
            </div>
