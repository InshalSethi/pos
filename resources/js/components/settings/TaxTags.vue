<template>
  <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 font-sans">
    ...
    <div class="fixed top-5 right-5 z-55 flex flex-col gap-3 max-w-sm w-full pointer-events-none">
      <transition-group
      ...
      >
        <div 
          ...
          :key="alert.id"
          class="pointer-events-auto w-full overflow-hidden rounded-2xl bg-white dark:bg-slate-900 border shadow-2xl p-4 flex items-start gap-3"
          :class="[
        ...
        >
          <div class="flex-shrink-0">
            <svg v-if="alert.type === 'error'" class="h-6 w-6 text-rose-600 dark:text-rose-450" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="flex-1 pt-0.5">
            <p class="text-xs font-bold" :class="alert.type === 'error' ? 'text-rose-905 dark:text-rose-400' : 'text-emerald-905 dark:text-emerald-400'">
              {{ alert.title }}
            </p>
          ...
          <div class="flex-shrink-0 flex">
            <button @click="removeAlert(alert.id)" class="inline-flex rounded-md text-slate-400 hover:text-slate-550 focus:outline-none">
    ...
    <!-- Header Section -->
    <div class="mb-8">
      <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Tax and Tags Management</h1>
      <p class="text-xs text-slate-500 mt-1 font-medium">Create and configure tax codes and tags to apply to your products and variant selections.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <!-- LEFT COLUMN: TAXES -->
      <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-[680px]">
        <!-- Card Header -->
        <div class="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div class="flex items-center gap-2.5">
            <h2 class="text-lg font-bold text-slate-900 dark:text-white">Taxes</h2>
          ...
          <button 
            type="button" 
            @click="openTaxModal()" 
            class="h-[34px] px-4 py-1.5 text-xs font-bold text-white bg-indigo-650 hover:bg-indigo-700 rounded-xl shadow-sm transition-all flex items-center gap-1.5 active:scale-95 cursor-pointer"
          >
        ...
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
          <div class="relative w-full">
            ...
            <input 
              ...
              class="w-full pl-9 pr-4 py-2 bg-slate-100 dark:bg-slate-850 dark:text-white border border-transparent rounded-xl text-xs font-semibold focus:outline-none focus:border-slate-300 focus:bg-white focus:ring-1 focus:ring-indigo-500 transition-all placeholder:text-slate-400"
            />
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
          <div v-if="loadingTaxes" class="text-center py-20">
            ...
            <p class="mt-3 text-slate-400 text-xs font-medium">Loading taxes...</p>
          </div>

          <div v-else-if="filteredTaxes.length === 0" class="text-center py-16 px-4">
            <div class="w-12 h-12 bg-slate-50 dark:bg-slate-850 rounded-full flex items-center justify-center mx-auto mb-3 border border-slate-100 dark:border-slate-800">
            ...
            <h3 class="text-sm font-bold text-slate-800 dark:text-white">No taxes found</h3>
            <p class="text-2xs text-slate-450 mt-1 max-w-xs mx-auto">Taxes allow billing system rules to automatically add sales rates on checkouts.</p>
          </div>

          <table v-else class="w-full table-auto border-collapse">
            <thead>
              <tr class="bg-slate-50 dark:bg-slate-850 border-b border-slate-200 dark:border-slate-800 text-[10px] font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-400 sticky top-0 z-10">
                ...
                <td class="px-6 py-4 text-center">
                  <span 
                    :class="[
                      ...
                      tax.is_active ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400' : 'bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-400'
                    ]"
                  >
                ...
                <td class="px-6 py-4 text-center align-middle">
                  <div class="flex items-center justify-center gap-1.5">
                    ...
                    <button 
                      ...
                      class="p-1.5 text-slate-450 hover:text-rose-600 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-all focus:outline-none cursor-pointer"
                      title="Delete Tax"
                    >
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-[680px]">
        ...
        <div class="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div class="flex items-center gap-2.5">
            <h2 class="text-lg font-bold text-slate-900 dark:text-white">Tags</h2>
            <span class="bg-indigo-50 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-400 text-2xs px-2 py-0.5 rounded-full font-bold uppercase">
              {{ tags.length }} Created
            </span>
          </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
          <div v-if="loadingTags" class="text-center py-20">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600 mx-auto"></div>
            <p class="mt-3 text-slate-400 text-xs font-medium">Loading tags...</p>
          </div>
          ...
          <div v-else-if="filteredTags.length === 0" class="text-center py-16 px-4">
            ...
            <h3 class="text-sm font-bold text-slate-800 dark:text-white">No tags found</h3>
            <p class="text-2xs text-slate-450 mt-1 max-w-xs mx-auto">Tags are identifiers that can be attached to item categories or standard/variation options.</p>
          </div>
          ...
          <table v-else class="w-full table-auto border-collapse">
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
              ...
              >
                <td class="px-6 py-4">
                  <div class="font-bold text-slate-900 dark:text-white text-xs">
                    <span class="inline-block px-2.5 py-1 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold border border-slate-200 dark:border-slate-700/60">
                      # {{ tag.name }}
                    </span>
                  </div>
                </td>
      ...
      <div v-if="showTaxModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-slate-900 w-full max-w-md p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl space-y-4 z-10 animate-in fade-in zoom-in-95 duration-200">
          <div class="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-slate-800">
            ...
            <button @click="closeTaxModal" class="text-slate-400 hover:text-slate-650 dark:hover:text-slate-200 font-bold text-lg focus:outline-none">&times;</button>
          </div>

          <form @submit.prevent="submitTax" class="space-y-4 text-xs font-medium">
            <div>
              <label class="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Tax Code / Name *</label>
              <input 
                v-model="taxForm.name"
                type="text" 
                placeholder="e.g., VAT, GST 18%, Sales Tax" 
                class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-850 dark:text-white border border-slate-250 dark:border-slate-800 rounded-xl focus:outline-none focus:border-slate-350 focus:bg-white focus:ring-1 focus:ring-indigo-500"
                required
              />
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Rate / Value *</label>
                <input 
              ...
              <div>
                <label class="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Value Type *</label>
                <select 
                  v-model="taxForm.type"
                  class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-850 dark:text-white border border-slate-250 dark:border-slate-800 rounded-xl focus:outline-none focus:border-slate-350 focus:bg-white focus:ring-1 focus:ring-indigo-500 cursor-pointer font-semibold"
                >
            ...
            <div class="flex items-center justify-between bg-slate-50 dark:bg-slate-850 p-3 rounded-2xl border border-slate-200/50 dark:border-slate-800/80">
              <div>
                ...
                <p class="text-[9px] text-slate-400 mt-0.5">Toggle to enable/disable applying this tax rate on billing</p>
              </div>
              <input 
                v-model="taxForm.is_active"
                type="checkbox" 
                class="w-4.5 h-4.5 rounded border-slate-350 text-indigo-650 focus:ring-indigo-500 cursor-pointer"
              />
            </div>
      ...
      <div v-if="showTagModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-slate-900 w-full max-w-md p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl space-y-4 z-10 animate-in fade-in zoom-in-95 duration-200">
          <div class="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-slate-800">
            ...
            <button @click="closeTagModal" class="text-slate-400 hover:text-slate-650 dark:hover:text-slate-200 font-bold text-lg focus:outline-none">&times;</button>
          </div>

          <form @submit.prevent="submitTag" class="space-y-4 text-xs font-medium">
            <div>
              <label class="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Tag Name *</label>
              <input 
