<template>
  <div class="w-full max-w-full py-8 px-4 sm:px-6 lg:px-8 dark:bg-zinc-950 min-h-screen">
    ...
    <div class="flex justify-between items-center mb-6">
      <div>
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-zinc-100 tracking-tight">Supplier Management</h1>
        <p class="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">Manage your supplier database and relationships</p>
      </div>
      <button
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
      <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl p-5 shadow-soft">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-[10px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">Total Suppliers</p>
            <p class="text-2xl font-extrabold text-slate-800 dark:text-zinc-100 mt-1">{{ formatNumber(statistics.total_suppliers || 0) }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl p-5 shadow-soft">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-[10px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">Active Suppliers</p>
            <p class="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400 mt-1">{{ formatNumber(statistics.active_suppliers || 0) }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl p-5 shadow-soft">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-[10px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">Inactive Suppliers</p>
            <p class="text-2xl font-extrabold text-rose-500 dark:text-rose-400 mt-1">{{ formatNumber((statistics.total_suppliers || 0) - (statistics.active_suppliers || 0)) }}</p>
          </div>
    ...
    <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-soft">
      <div class="flex flex-col sm:flex-row items-center justify-between p-4 gap-3 border-b border-slate-100 dark:border-zinc-800">
        ...
        <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          ...
          <div class="flex items-center bg-slate-100 dark:bg-zinc-800 p-0.5 rounded-lg text-[11px] font-semibold">
            <button
              type="button"
              @click="statusFilter = ''; loadSuppliers(1);"
              :class="statusFilter === '' ? 'bg-white dark:bg-zinc-700 text-slate-800 dark:text-zinc-100 shadow-xs' : 'text-slate-500 dark:text-zinc-400 hover:text-slate-700 dark:hover:text-zinc-300'"
            ...
            <button
              type="button"
              @click="statusFilter = '1'; loadSuppliers(1);"
              :class="statusFilter === '1' ? 'bg-white dark:bg-zinc-700 text-slate-800 dark:text-zinc-100 shadow-xs' : 'text-slate-500 dark:text-zinc-400 hover:text-slate-700 dark:hover:text-zinc-300'"
              class="px-3 py-1 rounded-md transition-all cursor-pointer"
            >
              Active
            </button>
            <button
              type="button"
              @click="statusFilter = '0'; loadSuppliers(1);"
              :class="statusFilter === '0' ? 'bg-white dark:bg-zinc-700 text-slate-800 dark:text-zinc-100 shadow-xs' : 'text-slate-500 dark:text-zinc-400 hover:text-slate-700 dark:hover:text-zinc-300'"
              class="px-3 py-1 rounded-md transition-all cursor-pointer"
            >
              Inactive
            </button>
        ...
        <div class="flex items-center space-x-2 text-xs text-slate-500 dark:text-zinc-400">
          ...
          <span>of {{ suppliers.total || 0 }} results</span>
        </div>
      </div>
        ...
        <table class="w-full text-left text-xs border-collapse">
          <thead class="sticky top-0 z-10 bg-slate-50 dark:bg-zinc-800">
            <tr class="bg-slate-50 dark:bg-zinc-800 border-b border-slate-200 dark:border-zinc-700 text-slate-500 dark:text-zinc-400 uppercase font-bold tracking-wider">
              ...
              <th class="py-3 px-4 bg-slate-50 dark:bg-zinc-800/50">Terms</th>
              <th class="py-3 px-4 text-right bg-slate-50 dark:bg-zinc-800/50">Credit Limit</th>
              <th class="py-3 px-4 text-right bg-slate-50 dark:bg-zinc-800/50">Advance</th>
          ...
          <tbody class="divide-y divide-slate-100/70 dark:divide-zinc-800">
            <tr v-if="loading" class="bg-white dark:bg-zinc-900">
              <td colspan="10" class="py-12 text-center text-slate-400 dark:text-zinc-500">
                <div class="flex flex-col items-center justify-center space-y-2">
                  ...
                  <span class="text-xs font-semibold">Loading suppliers...</span>
                </div>
              </td>
            </tr>
            <tr v-else-if="!suppliers.data || suppliers.data.length === 0" class="bg-white dark:bg-zinc-900">
              <td colspan="10" class="py-16 text-center text-slate-400 dark:text-zinc-500 italic">
                <svg class="mx-auto h-10 w-10 text-slate-300 dark:text-zinc-600 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                </svg>
                <span>No suppliers found. Get started by adding your first supplier.</span>
              </td>
            </tr>
            <tr v-else v-for="item in suppliers.data" :key="item.id" class="hover:bg-slate-50/50 dark:hover:bg-zinc-800/50 transition-colors">
              <!-- Supplier -->
              <td class="py-3.5 px-4 align-middle bg-white dark:bg-zinc-900">
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div class="text-[10px] text-slate-400 dark:text-zinc-500 mt-0.5">{{ item.company_name || 'No company' }} | ID: #{{ item.id }}</div>
                  </div>
                </div>
              </td>
              <!-- Contact -->
              ...
              <!-- Terms -->
              <td class="py-3.5 px-4 align-middle text-slate-700 dark:text-zinc-200 text-xs bg-white dark:bg-zinc-900">
              ...
              <td class="py-3.5 px-4 text-center relative align-middle bg-white dark:bg-zinc-900">
                ...
                >
                  <button @click="viewSupplier(item)" class="w-full text-left px-3 py-1.5 text-xs text-slate-700 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-800 flex items-center space-x-1.5 cursor-pointer">
                    <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                    <span>View</span>
                  </button>
                  <button @click="editSupplier(item)" class="w-full text-left px-3 py-1.5 text-xs text-slate-700 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-800 flex items-center space-x-1.5 cursor-pointer">
                  ...
                  <button @click="deleteSupplier(item)" class="w-full text-left px-3 py-1.5 text-xs text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 flex items-center space-x-1.5 cursor-pointer">
      ...
      <div v-else class="p-5">
        ...
        <div v-else-if="!suppliers.data || suppliers.data.length === 0" class="py-16 text-center text-slate-400 dark:text-zinc-500 italic">
          <svg class="mx-auto h-10 w-10 text-slate-300 dark:text-zinc-600 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
          </svg>
          <span>No suppliers found. Get started by adding your first supplier.</span>
            ...
            <div>
              ...
              <div class="flex items-start justify-between mb-4">
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-zinc-400 font-medium line-clamp-1">{{ item.company_name || 'No Company' }}</p>
              ...
              <div class="grid grid-cols-3 gap-2 py-3 text-center border-b border-slate-100 dark:border-zinc-800">
                ...
                <div class="bg-amber-50/60 dark:bg-amber-900/20 p-2 rounded-lg">
                  <p class="text-[10px] text-amber-600 dark:text-amber-400 font-bold uppercase">Advance</p>
                  <p class="text-xs font-extrabold text-amber-700 dark:text-amber-300 mt-0.5">{{ currencySymbol }}{{ formatNumber(item.advance_balance || 0) }}</p>
                </div>
      ...
      <div class="flex items-center justify-between p-4 border-t border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-900">
        <div class="text-xs text-slate-500 dark:text-zinc-400">
          Page {{ suppliers.current_page || 1 }} of {{ suppliers.last_page || 1 }}
        </div>
        <div class="flex items-center space-x-1">
          <button
          ...
          <template v-for="page in visiblePages" :key="page">
            <button
              ...
              :class="page === suppliers.current_page ? 'z-10 bg-slate-100 dark:bg-zinc-800 border-slate-300 dark:border-zinc-600 text-slate-800 dark:text-zinc-100' : 'bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-700 text-slate-500 dark:text-zinc-400 hover:bg-slate-50 dark:hover:bg-zinc-800'"
            >
              {{ page }}
            </button>
          </template>
  ...
  setup() {
    ...
    const loadSuppliers = async (page = 1) => {
      ...
      try {
        ...
        showToast('Error loading suppliers: ' + (error.response?.data?.message || error.message), 'error');
      } finally {
        loading.value = false;
      }
    };
    ...
    const debouncedSearch = debounce(() => { loadSuppliers(1); }, 300);
    ...
    const visiblePages = computed(() => {
      const current = suppliers.value.current_page;
      const last = suppliers.value.last_page;
      const pages = [];
    ...
    const handleSupplierSaved = () => {
      loadSuppliers(suppliers.value.current_page);
      loadStatistics();
      closeModal();
    };
    ...
    return {
      ...
      selectedSupplier, showCreateModal, showEditModal, showViewModal, showLedgerModal, viewMode,
      showLightbox, lightboxSrc, lightboxTitle,
      visiblePages, loadSuppliers, debouncedSearch, changePage, toggleActionDropdown,
      handleCreateSupplier, viewSupplier, editSupplier, viewLedger, deleteSupplier,
      closeModal, handleSupplierSaved, formatNumber, getInitials,
