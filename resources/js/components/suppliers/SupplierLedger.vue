<template>
  <div v-if="show" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-y-auto h-full w-full bg-slate-900/40 dark:bg-zinc-950/80 backdrop-blur-md transition-all duration-200" style="backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px);">
    <div class="relative mx-auto border border-slate-200 dark:border-zinc-800 w-full max-w-6xl shadow-2xl rounded-2xl bg-white dark:bg-zinc-900 text-slate-800 dark:text-slate-100 p-6 transition-all duration-300 z-10 max-h-[90vh] overflow-y-auto my-auto">
      <!-- Header -->
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6" v-if="ledgerData">
        ...
        <div class="bg-orange-50 p-4 rounded-lg">
          ...
          <p class="text-2xl font-bold text-orange-900">{{ currencySymbol }}{{ formatAmount(ledgerData.summary?.total_purchases || 0) }}</p>
        </div>
