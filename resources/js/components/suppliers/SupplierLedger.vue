  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        ...
        <div class="px-6 py-4 border-b border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shrink-0">
          <div class="flex items-center space-x-3">
            ...
            <div>
              <div class="flex items-center space-x-2">
                <h3 class="text-lg font-black text-slate-900 dark:text-zinc-100 tracking-tight">Supplier General Ledger</h3>
              ...
              <p class="text-xs font-semibold text-slate-500 dark:text-zinc-400">
                Accounts Payable Ledger for {{ activeSupplier?.name || supplier?.name }}
              </p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
            ...
            <button
              ...
              title="Export Supplier General Ledger PDF Statement"
            >
        ...
        <!-- Modal Body (Supplier Account Info Sidebar + General Ledger Table Grid) -->
        <div class="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 bg-slate-50/50 dark:bg-black/20 space-y-4">
          
          <!-- EXECUTIVE FINANCIAL STATUS BANNER (CRYSTAL CLEAR ACCOUNTS PAYABLE SUMMARY) -->
          <div
          ...
          >
            <div class="flex items-start space-x-3">
              ...
              <div>
                <h5 class="text-xs font-black uppercase tracking-wider text-rose-700 dark:text-rose-300">
                  Payment Payable Action Required
                </h5>
                <p class="text-xs font-medium text-rose-700 dark:text-rose-300 mt-0.5">
                  You have an outstanding balance of <strong class="text-sm font-black text-rose-800 dark:text-rose-100 underline underline-offset-2">{{ formatCurrency(closingBalance) }}</strong> payable to {{ activeSupplier?.name || supplier?.name }}.
                </p>
              </div>
            </div>
            <div class="shrink-0 self-end sm:self-center">
              <span class="px-3.5 py-1.5 rounded-xl text-xs font-black bg-rose-600 text-white shadow-xs">
                NET PAYABLE DUE: {{ formatCurrency(closingBalance) }}
              </span>
            </div>
          </div>
            ...
            <div class="flex items-start space-x-3">
              ...
              <div>
                <h5 class="text-xs font-black uppercase tracking-wider text-emerald-700 dark:text-emerald-300">
                  Supplier Advance / Debit Balance Credit
                </h5>
                <p class="text-xs font-medium text-emerald-700 dark:text-emerald-300 mt-0.5">
                  You have an advance overpayment / debit credit of <strong class="text-sm font-black text-emerald-800 dark:text-emerald-100">{{ formatCurrency(Math.abs(closingBalance)) }}</strong> with {{ activeSupplier?.name || supplier?.name }}. No payment is currently due.
                </p>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div>
              ...
              <p class="text-xs font-medium text-slate-600 dark:text-zinc-400 mt-0.5">
                All purchase invoices for {{ activeSupplier?.name || supplier?.name }} have been paid in full. No payment is due.
              </p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <!-- LEFT SIDE: Supplier Account Information Sidebar (col-span-3) -->
              ...
              <div class="bg-white dark:bg-zinc-900 rounded-2xl p-5 border border-slate-200/80 dark:border-zinc-800 shadow-xs space-y-5">
                ...
                <div class="text-center pb-4 border-b border-slate-100 dark:border-zinc-800">
                  ...
                  <h4 class="font-extrabold text-slate-900 dark:text-zinc-100 text-base leading-tight">
                    {{ activeSupplier?.name || supplier?.name || 'Supplier Ledger' }}
                  </h4>
                  <p v-if="activeSupplier?.company_name" class="text-xs text-slate-500 dark:text-zinc-400 font-medium">
                ...
                <div class="space-y-3 text-xs">
                  ...
                  <div class="flex items-start space-x-3">
                    ...
                    <div class="min-w-0 flex-1">
                      <p class="text-[10px] font-extrabold uppercase text-slate-400 dark:text-zinc-500">Address</p>
                      <p class="font-medium text-slate-800 dark:text-zinc-200 leading-snug">
                        {{ formatAddress(activeSupplier || supplier) }}
                      </p>
                    </div>
                  </div>
                  ...
                  <div class="pt-3 border-t border-slate-100 dark:border-zinc-800 space-y-2.5">
                    ...
                    <div class="flex items-start space-x-3">
                      ...
                      <div>
                        ...
                        <p class="font-extrabold text-slate-900 dark:text-zinc-100">
                          {{ formatCurrency(activeSupplier?.credit_limit || supplier?.credit_limit || 0) }}
                        </p>
                      </div>
                    </div>
                    ...
                    <div class="flex items-start space-x-3">
                      ...
                      <div>
                        <p class="text-[10px] font-extrabold uppercase text-slate-400 dark:text-zinc-500">Advance Balance</p>
                        <p class="font-extrabold text-emerald-600 dark:text-emerald-400">
                          {{ formatCurrency(activeSupplier?.advance_balance || supplier?.advance_balance || 0) }}
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div class="lg:col-span-9 space-y-5">
              ...
              <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <!-- Total Billed / Purchases (Credit - GREEN) -->
                <div class="bg-gradient-to-br from-emerald-500/10 via-emerald-500/5 to-transparent dark:from-emerald-500/15 dark:via-emerald-500/5 p-4 rounded-2xl border border-emerald-200 dark:border-emerald-900/60 shadow-2xs space-y-1">
                  <span class="text-[10px] font-extrabold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Total Purchases (Credit)</span>
                  <p class="text-base font-black text-emerald-600 dark:text-emerald-400">
                    {{ formatCurrency(totalCredits) }}
                  </p>
                </div>

                <!-- Total Paid / Returned (Debit - RED) -->
                <div class="bg-gradient-to-br from-rose-500/10 via-rose-500/5 to-transparent dark:from-rose-500/15 dark:via-rose-500/5 p-4 rounded-2xl border border-rose-200 dark:border-rose-900/60 shadow-2xs space-y-1">
                  <span class="text-[10px] font-extrabold uppercase tracking-wider text-rose-600 dark:text-rose-400">Total Paid / Returned (Debit)</span>
                  <p class="text-base font-black text-rose-600 dark:text-rose-400">
                    {{ formatCurrency(totalDebits) }}
                  </p>
                </div>

                <!-- NET AMOUNT OUTSTANDING TO PAY (DUE) -->
                <div
                  :class="[
                ...
                >
                  ...
                  >
                    {{ closingBalance > 0 ? 'Net Payable Due' : (closingBalance < 0 ? 'Supplier Advance' : 'Account Settled') }}
                  </span>
                  <p
                    :class="[
                ...
                <div class="p-4 border-b border-slate-100 dark:border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-50/50 dark:bg-zinc-900/50">
                  <div class="relative w-full sm:w-80">
                    ...
                    <input
                      v-model="searchQuery"
                      type="text"
                      placeholder="Search ref # (e.g. PO-1001), particulars..."
                  ...
                  <table class="w-full text-xs text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800">
                      ...
                      <tr v-if="dateRange?.start" class="bg-slate-50/80 dark:bg-zinc-800/40 font-bold">
                        ...
                        <td class="py-2.5 px-3 text-right font-black text-rose-600 dark:text-rose-400 border-r border-slate-100 dark:border-zinc-800">
                          {{ openingBalance < 0 ? `${formatCurrency(Math.abs(openingBalance))} Dr` : '-' }}
                        </td>
                        <td class="py-2.5 px-3 text-right font-black text-emerald-600 dark:text-emerald-400">
                          {{ openingBalance >= 0 ? `${formatCurrency(openingBalance)} Cr` : '-' }}
                        </td>
                      </tr>
                      ...
                      <tr v-else-if="paginatedTransactions.length === 0">
                        <td colspan="7" class="py-12 text-center text-slate-400 dark:text-zinc-500">
                          <p class="font-bold text-xs">No ledger transactions recorded for this supplier in selected period.</p>
                        </td>
                      </tr>
                      ...
                      <template v-else v-for="tx in paginatedTransactions" :key="tx.id">
                        <tr class="hover:bg-slate-50/80 dark:hover:bg-zinc-800/50 transition-colors">
                              ...
                              <div>
                                <div class="flex items-center space-x-2">
                                  ...
                                  <span
                                    :class="[
                                      ...
                                      tx.status === 'Paid' || tx.status === 'Paid & Posted' || tx.status === 'Posted'
                                        ? 'bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800'
                                        : (tx.status === 'Partially Paid'
                                          ? 'bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800'
                                          : (tx.status === 'Returned & Credited'
                                ...
                                <div class="flex items-center space-x-2 text-[10px] text-slate-400 dark:text-zinc-500 mt-0.5">
                                  ...
                                  <span v-else-if="tx.paid_amount > 0" class="text-emerald-600 dark:text-emerald-400 font-bold">
                                    Settled: {{ formatCurrency(tx.paid_amount) }}
                                  </span>
                                  <span v-if="tx.purchaser && tx.purchaser !== '-'">• Purchaser: {{ tx.purchaser }}</span>
                                </div>
                              </div>
                          ...
                          <!-- Transactions Debit (Paid / Returned - RED) -->
                          ...
                          <!-- Transactions Credit (Purchases + GREEN) -->
                          <td class="py-3 px-3 text-right font-black whitespace-nowrap border-r border-slate-100 dark:border-zinc-800">
                          ...
                          <td class="py-3 px-3 text-right font-black text-rose-600 dark:text-rose-400 whitespace-nowrap border-r border-slate-100 dark:border-zinc-800">
                            <span v-if="tx.balance < 0">{{ formatCurrency(Math.abs(tx.balance)) }} <span class="text-[9px] text-rose-500">Dr</span></span>
                            <span v-else class="text-slate-300 dark:text-zinc-700">-</span>
                          </td>

                          <!-- Balance Credit ($ GREEN Cr) -->
                          <td class="py-3 px-3 text-right font-black text-emerald-600 dark:text-emerald-400 whitespace-nowrap">
                            <span v-if="tx.balance >= 0">{{ formatCurrency(tx.balance) }} <span class="text-[9px] text-emerald-500">Cr</span></span>
                            <span v-else class="text-slate-300 dark:text-zinc-700">-</span>
                          </td>
                        </tr>
                    ...
                    <tfoot v-if="filteredTransactions.length > 0" class="bg-slate-900 text-white dark:bg-zinc-950 dark:text-zinc-100 font-black border-t-2 border-slate-800">
                      <tr>
                        ...
                        <td class="py-3 px-3 text-right text-rose-400 text-xs border-r border-slate-800">
                          {{ closingBalance < 0 ? `${formatCurrency(Math.abs(closingBalance))} Dr` : '-' }}
                        </td>
                        <td class="py-3 px-3 text-right text-emerald-400 text-xs">
                          {{ closingBalance >= 0 ? `${formatCurrency(closingBalance)} Cr` : '-' }}
                        </td>
                      </tr>
                    </tfoot>
                  </table>
        ...
        <div class="px-6 py-3.5 bg-slate-50/80 dark:bg-zinc-900 border-t border-slate-200/80 dark:border-zinc-800 flex items-center justify-between">
          <p class="text-xs text-slate-500 dark:text-zinc-400 font-medium">
            IAS 1 Compliant Accounts Payable General Ledger Statement — <strong class="text-slate-700 dark:text-zinc-300">{{ transactions.length }} Total Ledger Entries</strong>
          </p>
          <button
            @click="$emit('close')"
...
const accountType = ref('Liability (Creditors / Payable)');
const openingBalance = ref(0);
const closingBalance = ref(0);
const closingBalanceType = ref('Cr');
...
const sortOrder = ref('desc'); // Default to DESC (Newest First)
...
const fetchLedgerData = async () => {
  const suppId = props.supplier?.id;
  if (!suppId) return;
  ...
  try {
    ...
    const res = await api.get(`/suppliers/${suppId}/ledger`, { params });
    const data = res.data;

    activeSupplier.value = data.supplier || props.supplier;
    accountCode.value = data.supplier?.account_code || `AP-${String(suppId).padStart(5, '0')}`;
    accountType.value = data.supplier?.account_type || 'Liability (Creditors / Payable)';
    openingBalance.value = data.opening_balance || 0;
    closingBalance.value = data.closing_balance || 0;
    closingBalanceType.value = data.closing_balance_type || 'Cr';
...
const filteredTransactions = computed(() => {
  ...
  if (searchQuery.value.trim()) {
    ...
    list = list.filter(tx => 
      ...
      (tx.purchaser && tx.purchaser.toLowerCase().includes(q)) ||
...
const downloadPDF = async () => {
  const suppId = activeSupplier.value?.id || props.supplier?.id;
  if (!suppId) return;
  ...
  try {
    ...
    const response = await api.get(`/suppliers/${suppId}/ledger/pdf`, {
      params,
      responseType: 'blob'
    });
    ...
    link.href = url;
    const suppName = (activeSupplier.value?.name || props.supplier?.name || 'supplier').replace(/[^a-zA-Z0-9_-]/g, '_');
    link.setAttribute('download', `supplier_general_ledger_${suppName}_${new Date().toISOString().slice(0,10)}.pdf`);
    document.body.appendChild(link);
    link.click();
    link.remove();
    window.URL.revokeObjectURL(url);
  } catch (err) {
    console.error('Failed to export supplier ledger PDF:', err);
...
const formatAddress = (supp) => {
  if (!supp) return 'N/A';
  const parts = [supp.address, supp.city, supp.state, supp.country].filter(Boolean);
