<template>
  <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8 font-sans">
    ...
    <!-- Breadcrumb & Title -->
    <div class="mb-6">
      <router-link to="/inventory/transfer-orders" class="text-xs font-bold text-slate-400 hover:text-slate-650 flex items-center gap-1 mb-2">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7" />
        </svg>
        Back to Stock Transfers
      </router-link>
      <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Create Stock Transfer</h1>
    </div>
    ...
    <form @submit.prevent="submitTransfer" class="space-y-6 text-xs font-semibold text-slate-700">
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 space-y-4">
        <h3 class="text-sm font-extrabold text-slate-800 dark:text-white uppercase tracking-wider">Transfer Details</h3>
        
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <!-- Source Warehouse -->
          <div>
            <label class="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Source Warehouse *</label>
            <select 
              v-model="form.source_warehouse_id"
              @change="onSourceWarehouseChange"
              class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-850 dark:text-white border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-1 focus:ring-indigo-500 font-medium"
              required
            >
          ...
          <div>
            <label class="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Destination Warehouse *</label>
            <select 
          ...
          <!-- Transfer Date -->
          <div>
            <label class="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Transfer Date *</label>
            <input 
        ...
        <!-- Notes -->
        <div>
          <label class="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Notes / Purpose of Transfer</label>
          <textarea 
            v-model="form.notes"
            rows="2"
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 space-y-4">
        <div class="flex justify-between items-center">
          <h3 class="text-sm font-extrabold text-slate-800 dark:text-white uppercase tracking-wider">Items to Transfer</h3>
          <span class="text-xs text-slate-400">Add products from dropdown to configure transfer amounts</span>
        </div>
        ...
        <div class="flex gap-2">
          ...
          <button 
            ...
            class="px-4 py-2.5 bg-slate-850 dark:bg-indigo-650 hover:bg-slate-900 text-white rounded-xl font-bold transition-all disabled:opacity-50"
          >
            Add
          </button>
        </div>
          ...
          <table class="w-full table-auto border-collapse text-xs">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
              <tr v-if="form.items.length === 0">
                <td colspan="4" class="px-4 py-8 text-center text-slate-400 italic">No products added yet. Choose a source warehouse and add items to list.</td>
              </tr>
              <tr v-for="(item, idx) in form.items" :key="idx" class="hover:bg-slate-50/40">
                <!-- Product Details -->
                <td class="px-4 py-3">
                  <div class="font-bold text-slate-900 dark:text-white">{{ item.product_name }}</div>
                  <div v-if="item.variation_name" class="text-[10px] text-slate-400 font-semibold mt-0.5">{{ item.variation_name }}</div>
                  <div class="text-[9px] text-slate-400 mt-0.5 font-medium">SKU: {{ item.sku }}</div>
                </td>
                <!-- Available Stock -->
                ...
                <td class="px-4 py-3 text-center">
                  <div class="flex flex-col items-center">
                    <input 
                      ...
                      class="w-24 px-2 py-1 text-center bg-slate-50 dark:bg-slate-850 dark:text-white border rounded-lg focus:outline-none font-bold text-xs"
                      :class="item.quantity > item.available_qty ? 'border-rose-450 text-rose-600 focus:ring-1 focus:ring-rose-400' : 'border-slate-200 dark:border-slate-800 focus:ring-1 focus:ring-indigo-500'"
                    />
                ...
                <td class="px-4 py-3 text-center">
                  <button 
                    ...
                    class="p-1 text-slate-400 hover:text-rose-600 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-850 transition-all focus:outline-none"
                  >
      ...
      <!-- Action Buttons -->
      <div class="flex justify-between items-center">
        <span class="text-[11px] text-slate-400">Fields marked with (*) are required.</span>
        
        <div class="flex gap-2">
          <button 
            type="button" 
            @click="$router.push('/inventory/transfer-orders')"
            class="px-5 py-2.5 text-slate-550 font-bold hover:text-slate-700"
          >
            Cancel
          </button>
          ...
          <button 
            ...
            class="px-6 py-2.5 bg-indigo-650 hover:bg-indigo-700 text-white rounded-full font-bold shadow-md hover:shadow-lg transition-all flex items-center gap-1.5 disabled:opacity-50"
          >
