  ...
  <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 font-sans">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
      <div class="flex items-center gap-3">
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Variants</h1>
        <!-- Hollow Star Option -->
        <button 
          type="button" 
          @click="isStarred = !isStarred" 
          class="text-slate-400 hover:text-amber-400 transition-colors focus:outline-none"
        >
          <svg 
            class="w-7 h-7" 
      ...
      <div class="flex items-center gap-3">
        ...
        <button 
          ...
          type="button" 
          @click="bulkDelete"
          class="px-4 py-2 text-sm font-semibold text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-xl border border-rose-200 transition-colors flex items-center gap-2"
        >
        ...
        <button 
          type="button" 
          @click="openModal" 
          class="px-5 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-full shadow-sm hover:shadow transition-all flex items-center gap-2 active:scale-95"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div v-if="loading" class="text-center py-20">
        ...
        <p class="mt-3 text-slate-500 text-sm font-medium">Loading variants...</p>
      </div>
      ...
      <div v-else-if="filteredAttributes.length === 0" class="text-center py-20 px-4">
        ...
        <h3 class="text-base font-bold text-slate-800">No variants found</h3>
        <p class="text-xs text-slate-400 mt-1 max-w-xs mx-auto">Create global attributes (e.g. Size, Color, Capacity) to manage product variation options globally.</p>
        <button 
          type="button" 
          @click="openModal"
          class="mt-4 px-4 py-2 text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition-all"
        >
          Add First Variant
        </button>
      </div>
        ...
        <table class="w-full table-auto border-collapse">
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            >
              ...
              <td class="px-6 py-4 align-middle font-bold text-slate-900">
                <div class="flex flex-col">
                  <span>{{ attr.name }}</span>
                  <span class="text-[10px] font-medium text-slate-400 tracking-tight mt-0.5">
                    {{ attr.values.map(v => v.value).join(', ') }}
                  </span>
                </div>
              </td>
              <!-- Values Column (Right-aligned count) -->
              <td class="px-6 py-4 align-middle text-right font-bold text-slate-600 text-sm">
                <span class="bg-slate-100 text-slate-700 px-2.5 py-1 rounded-full text-xs font-bold border border-slate-200/40">
                  {{ attr.values.length }}
                </span>
              </td>
              <!-- Actions -->
              <td class="px-6 py-4 align-middle text-center">
                <button 
                  ...
                  class="p-1.5 text-slate-400 hover:text-rose-600 rounded-lg hover:bg-rose-50 transition-all focus:outline-none"
    ...
    >
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <!-- Modal Box -->
        <div class="relative bg-white w-full max-w-md p-6 rounded-3xl border border-slate-200 shadow-2xl space-y-4 z-10 animate-in fade-in zoom-in-95 duration-200">
          <div class="flex justify-between items-center pb-2 border-b border-slate-100">
            <h3 class="text-sm font-extrabold text-slate-800 uppercase tracking-wider">Configure Global Variant</h3>
            <button @click="closeModal" class="text-slate-400 hover:text-slate-600 font-bold text-lg">&times;</button>
          </div>
          ...
          <form @submit.prevent="submitVariant" class="space-y-4">
            <div>
              <label class="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Option Name</label>
              <input 
            ...
            <div>
              <label class="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Option Values (Comma separated)</label>
              <input 
            ...
            <div class="flex justify-end gap-2 pt-2 text-xs">
              <button 
                type="button" 
                @click="closeModal" 
                class="px-4 py-2 text-slate-500 font-bold hover:text-slate-700"
              >
                Cancel
              </button>
              <button 
                type="submit" 
                :disabled="submitting" 
                class="px-5 py-2 bg-emerald-600 text-white font-bold rounded-xl shadow hover:bg-emerald-700 transition-colors flex items-center gap-1.5 disabled:opacity-50"
              >
