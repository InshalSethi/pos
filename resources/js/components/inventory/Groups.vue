<template>
  <div class="p-6 space-y-6">
    ...
    <div v-if="toast" class="p-4 rounded-xl border flex items-center justify-between text-xs font-semibold animate-in fade-in slide-in-from-top duration-300" :class="toast.type === 'success' ? 'bg-emerald-50 text-emerald-800 border-emerald-100 dark:bg-emerald-950/20 dark:text-emerald-300 dark:border-emerald-900/50' : 'bg-rose-50 text-rose-800 border-rose-100 dark:bg-rose-950/20 dark:text-rose-300 dark:border-rose-900/50'">
      ...
      <button @click="toast = null" class="text-gray-400 hover:text-gray-600 font-bold">&times;</button>
    </div>
      ...
      <div class="lg:col-span-8 space-y-6">
        <div v-if="selectedGroup" class="bg-white dark:bg-slate-900 border border-gray-150 dark:border-slate-800/80 rounded-[24px] p-6 shadow-sm space-y-6 animate-in fade-in duration-200">
          ...
          <div class="flex justify-between items-start gap-4 pb-4 border-b border-gray-100 dark:border-slate-800">
            ...
            <div class="flex gap-2">
              <button
                ...
                class="p-2 text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-850 rounded-xl transition-all cursor-pointer"
            ...
            <div class="p-5 bg-gradient-to-br from-indigo-50/20 to-white dark:from-slate-850 dark:to-slate-900 rounded-[20px] border border-gray-100 dark:border-slate-800 space-y-4">
              ...
              <div class="space-y-3">
                ...
                <div v-if="bulkAction.action_type !== 'set_tax'">
                  <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Percentage Value (%)</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
                ...
                <label class="flex items-center gap-2 py-1 cursor-pointer select-none text-[11px] font-semibold text-slate-550 dark:text-slate-400">
                  <input
                    type="checkbox"
                ...
                <button
                  ...
                  class="w-full py-2.5 bg-indigo-650 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all duration-150 shadow-md cursor-pointer flex items-center justify-center gap-1.5"
                >
          ...
          <div class="space-y-4 pt-4 border-t border-gray-100 dark:border-slate-800">
            <div class="flex items-center justify-between">
              <h3 class="text-xs font-bold text-slate-500 uppercase tracking-wider">Group Products Lookup</h3>
              ...
              <table class="min-w-full divide-y divide-gray-150 dark:divide-slate-800">
                <thead class="bg-slate-50 dark:bg-slate-850">
                  <tr>
                    ...
                    <th class="px-4 py-2.5 text-left text-[9px] font-black text-slate-400 uppercase tracking-wider">Name</th>
                    <th class="px-4 py-2.5 text-right text-[9px] font-black text-slate-400 uppercase tracking-wider">Cost Price</th>
                    <th class="px-4 py-2.5 text-right text-[9px] font-black text-slate-400 uppercase tracking-wider">Selling Price</th>
                    <th class="px-4 py-2.5 text-center text-[9px] font-black text-slate-400 uppercase tracking-wider">Tax Rate</th>
                    <th class="px-4 py-2.5 text-center text-[9px] font-black text-slate-400 uppercase tracking-wider">Discount</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-slate-800/60 bg-white dark:bg-slate-900 text-xs">
                  <tr v-for="prod in filteredProducts" :key="prod.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-850/30">
                    ...
                    <td class="px-4 py-2.5 text-right font-medium text-slate-500 dark:text-slate-450">${{ parseFloat(prod.cost_price).toFixed(2) }}</td>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm" role="dialog">
      <div class="relative bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-[28px] max-w-md w-full overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        ...
        <div class="p-6 border-b border-gray-100 dark:border-slate-800">
          ...
          <p class="text-[10px] text-gray-400 uppercase tracking-widest mt-0.5">Classification Parameters</p>
        </div>
        ...
        <form @submit.prevent="saveGroup" class="p-6 space-y-4 overflow-y-auto">
          <!-- Group Name -->
          <div>
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Group Name *</label>
            <input
          ...
          <div class="flex items-center justify-between py-2 border-t border-slate-100 dark:border-slate-800 mt-2">
