<template>
  <div class="p-6 space-y-6">
    ...
    <!-- Filters Section -->
    <div class="bg-white dark:bg-slate-900 border border-slate-100/80 dark:border-slate-800/50 rounded-[24px] p-5 shadow-xs space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <!-- Item Search -->
        <div>
          <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Item Search</label>
          <div class="relative">
            <input
              v-model="filters.search"
              type="text"
              placeholder="Name, SKU, or Barcode..."
              class="w-full pl-8 pr-3 py-1.5 text-xs bg-slate-50/50 dark:bg-slate-850 border border-slate-200/60 dark:border-slate-800/80 focus:border-indigo-500 dark:focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 rounded-xl outline-none text-slate-800 dark:text-slate-200 transition-all duration-150"
              @input="debouncedFetch"
            />
            <span class="absolute left-2.5 top-2 text-gray-455">
        ...
        <div>
          ...
          <select
            ...
            class="w-full px-3 py-1.5 text-xs bg-slate-50/50 dark:bg-slate-850 border border-slate-200/60 dark:border-slate-800/80 focus:border-indigo-500 dark:focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 rounded-xl outline-none text-slate-800 dark:text-slate-200 transition-all duration-150"
            @change="fetchHistory"
          >
    ...
    <!-- Master Timeline Log -->
    <div class="bg-white dark:bg-slate-900 border border-slate-100/80 dark:border-slate-800/50 rounded-[24px] overflow-hidden shadow-xs">
      ...
      <div v-if="loading" class="p-16 text-center space-y-3">
        ...
        <p class="text-xs text-gray-400 font-medium">Retrieving master inventory logs...</p>
      </div>

      <!-- Empty State -->
      <div v-else-if="logs.length === 0" class="p-16 text-center">
        <div class="text-4xl mb-3">📜</div>
        <h3 class="text-xs font-bold text-slate-750 dark:text-slate-300">No Inventory Movements</h3>
        ...
        <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-800/60">
          <thead class="bg-slate-50 dark:bg-slate-850/40">
            <tr>
              ...
              <th class="px-6 py-3.5 text-left text-[9px] font-black text-slate-400 uppercase tracking-wider">Item Name</th>
              <th class="px-6 py-3.5 text-left text-[9px] font-black text-slate-400 uppercase tracking-wider">Warehouse</th>
              <th class="px-6 py-3.5 text-center text-[9px] font-black text-slate-400 uppercase tracking-wider">Qty Change</th>
              <th class="px-6 py-3.5 text-center text-[9px] font-black text-slate-400 uppercase tracking-wider">Type / Reason</th>
            ...
            <tr v-for="log in logs" :key="log.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-850/20 transition-colors">
              ...
              <td class="px-6 py-3.5">
                <div class="min-w-[150px]">
                  ...
                  <p class="text-[9px] font-semibold text-slate-450 uppercase tracking-wider mt-0.5">
                    SKU: {{ log.product?.sku }}
              ...
              <!-- Warehouse -->
              <td class="px-6 py-3.5 text-slate-700 dark:text-slate-355 font-bold">
              ...
              <!-- Reference ID -->
              <td class="px-6 py-3.5 text-slate-550 dark:text-slate-400 font-bold whitespace-nowrap">
      ...
      <!-- Pagination Control -->
      <div v-if="pagination.last_page > 1" class="px-6 py-4 bg-slate-50 dark:bg-slate-850/20 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs">
        ...
        <div class="flex items-center gap-1.5">
          <button
            ...
            class="px-3 py-1.5 rounded-lg border border-slate-200/60 dark:border-slate-800/80 bg-white dark:bg-slate-800 disabled:opacity-50 text-slate-700 dark:text-slate-300 font-bold cursor-pointer transition-all hover:bg-slate-50"
          >
            Prev
          </button>
