    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full" @click.stop>
        <div class="bg-white">
          <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
            <div class="flex items-center justify-between">
              <div class="flex items-center space-x-4">
                ...
                <div v-if="adminData">
                  <h3 class="text-xl font-semibold text-white">{{ adminData.name }}</h3>
                  <p class="text-blue-100 text-sm">ID: #{{ adminData.id }}</p>
                </div>
              </div>
          ...
          <div v-else-if="adminData" class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              <div class="p-4 bg-blue-50 border border-blue-100 rounded-xl">
                <p class="text-xs text-blue-500 font-bold uppercase">Email</p>
                <p class="text-sm font-semibold text-gray-800 truncate">{{ adminData.email }}</p>
              </div>
              <div class="p-4 bg-purple-50 border border-purple-100 rounded-xl">
                <p class="text-xs text-purple-500 font-bold uppercase">Role</p>
                <p class="text-sm font-semibold text-gray-800 capitalize">{{ adminData.roles?.[0]?.name || 'No Role' }}</p>
              </div>
              <div class="p-4 bg-yellow-50 border border-yellow-100 rounded-xl">
                ...
                <p class="text-sm font-semibold text-gray-800">{{ formatDate(adminData.created_at) }}</p>
              </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-10">
                <div>
                  ...
                  <p class="text-gray-800 font-medium border-b border-gray-200 pb-1">{{ adminData.name }}</p>
                </div>
                <div>
                  <label class="text-xs font-semibold text-gray-500">Contact Number</label>
                  <p class="text-gray-800 font-medium border-b border-gray-200 pb-1">{{ adminData.phone || 'Not Provided' }}</p>
                </div>
                <div class="md:col-span-2">
                  <label class="text-xs font-semibold text-gray-500">Residential Address</label>
                  <p class="text-gray-800 font-medium border-b border-gray-200 pb-1">{{ adminData.address || 'No address on file' }}</p>
                </div>
                <div class="md:col-span-2">
                  ...
                  <div class="mt-2 text-sm text-gray-600 bg-white p-4 rounded-lg border italic">
                    "{{ adminData.notes || 'No additional notes.' }}"
                  </div>
                </div>
              </div>
            </div>
...
export default {
  name: 'SubAdminViewModal',
  props: {
    show: { type: Boolean, default: false },
    subAdmin: { type: Object, default: null } 
  },
  emits: ['close'],
  setup(props, { emit }) {
    ...
    const loadDetails = async () => {
      if (!props.subAdmin?.id) return;
      loading.value = true;
      try {
        const response = await api.get(`/sub-admins/${props.subAdmin.id}`);
        adminData.value = response.data;
      } catch (error) {
        console.error('Fetch Error:', error);
        // Fallback: use data already present in the list
        adminData.value = props.subAdmin;
      } finally {
        loading.value = false;
      }
    };
    ...
    return { loading, adminData, formatDate, closeModal };
  }
};
</script>