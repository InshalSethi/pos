    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transition-all text-left">
      ...
      <div class="p-6 text-center">
        ...
        <p class="text-xs text-zinc-500 dark:text-zinc-400 font-semibold mb-6">
          Are you sure you want to delete <span class="font-extrabold text-zinc-900 dark:text-white">"{{ formTitle }}"</span>? This custom form layout will no longer be applied to transactions.
        </p>
...
const formTitle = computed(() => props.formItem?.name || 'this Custom Form');
...
const confirmDelete = async () => {
  if (!props.formItem?.id || deleting.value) return;

  deleting.value = true;
  errorMessage.value = '';

  try {
    await axios.delete(`/admin/api/custom-forms/${props.formItem.id}`);
    emit('deleted');
    close();
  } catch (e) {
    console.error("Failed to delete custom form", e);
    errorMessage.value = e.response?.data?.message || 'Failed to delete custom form.';
  } finally {
